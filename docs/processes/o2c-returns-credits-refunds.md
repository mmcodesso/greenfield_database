...
This is one of the most useful student exception paths in the dataset because it shows that correcting revenue is both an operational process and an accounting process. The dataset keeps it as a minority path so students can compare normal sales with exception handling.
