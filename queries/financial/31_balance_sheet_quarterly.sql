-- Teaching objective: Produce a detailed quarterly classified balance sheet from posted general-ledger activity.
-- Main tables: GLEntry, Account, JournalEntry.
-- Output shape: One row per fiscal quarter and balance-sheet line.
-- Interpretation notes: The statement shows quarter-end balances, not quarterly activity. Interim equity includes a derived Current Year Earnings line so the statement ties before year-end close.
...
WHERE FiscalPeriod IN (3, 6, 9, 12)
ORDER BY
    FiscalYear,
    FiscalQuarter,
    DisplayOrder,
