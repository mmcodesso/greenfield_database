-- Teaching objective: Produce a detailed monthly direct-method cash flow statement from posted cash-ledger activity.
-- Main tables: GLEntry, Account, JournalEntry.
-- Output shape: One row per fiscal month and cash-flow statement line.
-- Interpretation notes: Cash flows are period activity, not ending balances. Opening journals establish beginning cash for the first reporting period and are excluded from operating, investing, and financing sections.
