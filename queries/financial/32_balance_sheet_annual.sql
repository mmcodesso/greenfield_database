-- Teaching objective: Produce a detailed annual classified balance sheet from posted general-ledger activity.
-- Main tables: GLEntry, Account, JournalEntry.
-- Output shape: One row per fiscal year and balance-sheet line.
-- Interpretation notes: The statement shows year-end balances, not annual activity. Equity relies on posted retained-earnings close and a derived Current Year Earnings line that should be zero after close.
