...
def test_posting_engine_posts_phase5_transactions() -> None:
    ...
    commission_expense_rows = gl[gl["AccountID"].astype(int).eq(accounts["6290"])]
    commission_payable_rows = gl[gl["AccountID"].astype(int).eq(accounts["2034"])]
    ...
    assert round(float(commission_expense_rows["Debit"].sum()) - float(commission_expense_rows["Credit"].sum()), 2) > 0
    assert round(float(commission_payable_rows["Credit"].sum()) - float(commission_payable_rows["Debit"].sum()), 2) > 0
