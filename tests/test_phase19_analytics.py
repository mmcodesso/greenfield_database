...
def test_phase19_docs_reference_new_query_and_case_flow() -> None:
    ...
    query_library = Path("docs/analytics/analysis-tracks.md").read_text(encoding="utf-8")
    ...
    assert "Query Library" in analytics_hub
    assert "Financial Queries" in query_library
    assert "Case-Support Trace Queries" in query_library
